...
// 이 buffer를 현재 'ELEMENT_ARRAY_BUFFER'로 만들기

    // initialize buffer
    // const data = await openSTLByUrl("assets/stl/Crown_23.stl");
    // vertices = data.getPoints().getData();
  ...
  const drawScene = function() {
    ...
    // gl.drawElements(primitiveType, indices.length, indexType, offset);
    gl.drawElements(gl.LINES, indices.length, indexType, offset);
